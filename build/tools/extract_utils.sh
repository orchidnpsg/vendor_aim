...
function setup_vendor() {
    ...
    export AIM_ROOT="$3"
    if [ ! -d "$AIM_ROOT" ]; then
        echo "\$AIM_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$AIM_ROOT/$OUTDIR" ]; then
        mkdir -p "$AIM_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$AIM_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$AIM_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$AIM_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$AIM_ROOT"/vendor/aim/build/tools/smali/baksmali.jar
        export SMALIJAR="$AIM_ROOT"/vendor/aim/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$AIM_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$AIM_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$AIM_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$AIM_ROOT"/vendor/aim/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
    ...
    for (( i=1; i<COUNT+1; i++ )); do
        ...
        if [ "$SRC" = "adb" ]; then
            ...
            if [ -f "$SRC/$FILE" ]; then
                cp "$SRC/$FILE" "$DEST"
            # if file does not exist try AIM target
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$AIM_ROOT"/"$OUTDIR"/radio
