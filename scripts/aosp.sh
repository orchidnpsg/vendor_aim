...
# is located in vendor/aim/scripts. Other ROMs will need to change this. The logic is
# as follows:
...
function get_repos() {
  ...
  do
    if grep -q "$i" /tmp/rebase.tmp; then # If Google has it and
      if grep -q "$i" $WORKING_DIR/manifest/snippets/aim.xml; then # If we have it in our manifest and
        if grep "$i" $WORKING_DIR/manifest/snippets/aim.xml | grep -q "remote="; then # If we track our own copy of it
