...
ifeq ($(AIM_BUILDTYPE), OFFICIAL)
    ifndef TARGET_VENDOR_RELEASE_BUILD_ID
        AIM_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d)-OFFICIAL-$(AIM_BUILD)-$(FINAL)
    else
        ifeq ($(TARGET_BUILD_VARIANT),user)
            ifeq ($(AIM_VERSION_MAINTENANCE),0)
                ...
                AIM_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(AIM_VERSION_MAINTENANCE)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-OFFICIAL-$(AIM_BUILD)-$(FINAL)
            endif
        else
            AIM_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-OFFICIAL-$(AIM_BUILD)-$(FINAL)
        endif
    endif
else
    ifeq ($(AIM_VERSION_MAINTENANCE),0)
        AIM_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d)-$(AIM_BUILDTYPE)$(AIM_EXTRAVERSION)-$(AIM_BUILD)-$(FINAL)
    else
        AIM_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(AIM_VERSION_MAINTENANCE)-$(shell date -u +%Y%m%d)-$(AIM_BUILDTYPE)$(AIM_EXTRAVERSION)-$(AIM_BUILD)-$(FINAL)
    endif
endif
...
ifneq ($(PRODUCT_DEFAULT_DEV_CERTIFICATE),build/target/product/security/testkey)
    ifneq ($(AIM_BUILDTYPE), UNOFFICIAL)
        ...
        ifeq ($(AIM_VERSION_MAINTENANCE),0)
            AIM_DISPLAY_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-$(AIM_BUILD)-$(FINAL)
        else
            AIM_DISPLAY_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(AIM_VERSION_MAINTENANCE)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-$(AIM_BUILD)-$(FINAL)
        endif
    endif
endif
endif
